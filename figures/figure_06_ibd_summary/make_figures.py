...
The script reads the AADR v62 all-SNP CROSS64 summary. Figure 6
combines the preselected asymmetric subset (full-UDG held raw, non-UDG
treatment varied) with same-treatment full-UDG/full-UDG and
non-UDG/non-UDG comparisons.  Figure S2 is the 16-point length-versus-count
diagnostic across the four library-pair categories.

Primary calls use strict >12 cM (and, for Figure S2 panel b, >20 cM) together
...
FIGURE6_PANELS = (
    (
        ...
        "ORT15_fu_x_ORT16_nu",
        "asymmetric",
        "ORT15 full-UDG (raw) × ORT16 non-UDG\n(non-UDG treatment varied)",
    ),
    (
        "b",
        "ORT15_nu_x_ORT16_fu",
        "asymmetric",
        "ORT15 non-UDG × ORT16 full-UDG (raw)\n(non-UDG treatment varied)",
    ),
    (
        "c",
        "ORT15_fu_x_ORT16_fu",
...
def figure6_index(
    ...
    if not needed_same_keys.issubset(same_index):
        ...
        raise ValueError(f"Missing same-treatment Figure 6 rows: {missing}")

    selected: Dict[Tuple[str, str], Mapping[str, str]] = {}
    for panel, direction, design, _ in FIGURE6_PANELS:
        source = asymmetric_index if design == "asymmetric" else same_index
        for method in METHODS:
            selected[(panel, method)] = source[(direction, method)]
    if len(selected) != 16:
        raise ValueError(f"Expected 16 Figure 6 rows, found {len(selected)}")
    return selected
...
def draw_figure6(
    ...
    indexed = figure6_index(asymmetric_rows, summary_rows)
    all_totals = [
        finite_float(row, "density220_sum_gt12_cM", "Figure 6")
    ...
    for axis, (panel, direction, _design, panel_title) in zip(
        ...
        for y_position, method, total, count in zip(
            ...
            place_left = total > x_min + 0.68 * (x_max - x_min)
            axis.annotate(
    ...
    figure.text(
        0.5,
        0.012,
        "a-b: full-UDG held raw; c-d: same treatment on both. Labels give rounded cM and retained-segment count.",
        ha="center",
        va="bottom",
        fontsize=7.0,
        color="#444444",
    ...
    metadata = {
        "Title": "Corrected four-pair IBD1 recovery across PMD treatments",
        "Author": "ORT study",
        "Subject": "AADR v62 all-SNP ancIBD; strict >12 cM and >220 SNP/cM",
        "Creator": "plot_fig6_fig7_v62.py",
    }
...
def draw_s2_diagnostic(rows: Sequence[Mapping[str, str]], output: Path) -> None:
    ...
    figure.legend(
        ...
        ncol=2,
        fontsize=7.2,
        title="Same treatment on both libraries",
        title_fontsize=7.4,
        frameon=False,
        handletextpad=0.45,
        columnspacing=1.2,
...
def write_validation(
    ...
    asymmetric_rows: Sequence[Mapping[str, str]],
    summary_rows: Sequence[Mapping[str, str]],
    figure6_xlim: Tuple[float, float],
) -> None:
    payload = {
        "status": "PASS",
        "analysis": "AADR v62 all-SNP corrected ancIBD",
        "filters": {
            "length": "strict >12 cM; Figure S2 also strict >20 cM",
        ...
        "design": {
            "figure6_panels": 4,
            "figure6_rows": len(figure6_index(asymmetric_rows, summary_rows)),
            "figure6_shared_x_limits_cM": list(figure6_xlim),
            "figure6_panel_definitions": [
                {
                    "panel": panel,
                    "library_direction": direction,
                    "design": design,
                }
                for panel, direction, design, _title in FIGURE6_PANELS
            ],
            "figureS2_same_treatment_rows": sum(
                row.get("same_treatment") == "yes" for row in summary_rows
            ),
        },
    }
    with (output_dir / "FIG6_S2_VALIDATION.json").open("w") as handle:
...
def write_display_values(
    ...
    selected_index = figure6_index(asymmetric_rows, summary_rows)
    with (output_dir / "FIG6_DISPLAY_VALUES.tsv").open("w", newline="") as handle:
        fields = (
            "panel",
            "design",
            "library_direction",
    ...
    with (output_dir / "FIGS2_DISPLAY_VALUES.tsv").open("w", newline="") as handle:
        fields = (
            "library_direction",
...
def main() -> None:
    ...
    figure6 = args.output_dir / "figure_06_ibd_summary.pdf"
    figure_s2 = args.output_dir / "supplementary_04_ibd_length_count.pdf"
    figure6_xlim = draw_figure6(asymmetric_rows, summary_rows, figure6)
    draw_s2_diagnostic(summary_rows, figure_s2)
    ...
    print(f"figure6={figure6}")
    print(f"figureS2={figure_s2}")
    print(f"callable_map_denominator_cM={denominator:.6f}")
    print("design_validation=PASS (4 Figure 6 panels/16 rows; 16 Figure S2 rows)")
